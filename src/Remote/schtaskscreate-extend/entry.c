...
)
{
	...
	wchar_t * taskpathcopy = NULL;
	const wchar_t * xml = NULL;
	int mode = 0;
	BOOL force = 0;
	wchar_t * hostnameList = NULL;
	wchar_t * currentHost = NULL;
	wchar_t * token = NULL;
	int successCount = 0;
	int failCount = 0;
	int totalHosts = 0;
	...
	// Create a working copy of the hostname string for tokenization
	size_t hostnameLen = MSVCRT$wcslen(hostname);
	hostnameList = (wchar_t *)intAlloc((hostnameLen + 1) * sizeof(wchar_t));
	if (NULL == hostnameList)
	{
		BeaconPrintf(CALLBACK_ERROR, "Failed to allocate memory for hostname list\n");
		goto go_end;
	}
	MSVCRT$wcscpy(hostnameList, hostname);

	// Manually parse comma-separated hostnames (avoiding wcstok nested call issue)
	wchar_t * current = hostnameList;
	wchar_t * commaPos = NULL;

	while (current != NULL && *current != L'\0')
	{
		// Find next comma
		commaPos = MSVCRT$wcschr(current, L',');

		// If found, null-terminate at comma position
		if (commaPos != NULL)
		{
			*commaPos = L'\0';
		}

		// Trim leading/trailing spaces from current token
		token = current;
		while (*token == L' ' || *token == L'\t') token++;

		if (MSVCRT$wcslen(token) > 0)
		{
			wchar_t * end = token + MSVCRT$wcslen(token) - 1;
			while (end > token && (*end == L' ' || *end == L'\t'))
			{
				*end = L'\0';
				end--;
			}
		...
		// Create a copy of taskpath for each iteration (createTask modifies it)
		size_t taskpathLen = MSVCRT$wcslen(taskpath);
		taskpathcopy = (wchar_t *)intAlloc((taskpathLen + 1) * sizeof(wchar_t));
		if (NULL == taskpathcopy)
		{
			BeaconPrintf(CALLBACK_ERROR, "Failed to allocate memory for taskpath copy\n");
			failCount++;
			if (commaPos == NULL)
				break;
			current = commaPos + 1;
		...
		MSVCRT$wcscpy(taskpathcopy, taskpath);

		internal_printf("createTask hostname:%S taskpath:%S mode:%d force:%d\n",
			token, taskpathcopy, mode, force);

		dwErrorCode = createTask(token, taskpathcopy, xml, mode, force);
		if(ERROR_SUCCESS != dwErrorCode)
		{
			BeaconPrintf(CALLBACK_ERROR, "createTask failed for host %S: %lX\n", token, dwErrorCode);
			failCount++;
		}
		else
		{
			internal_printf("SUCCESS for host: %S\n", token);
			successCount++;
		}

		if (taskpathcopy)
		{
			intFree(taskpathcopy);
			taskpathcopy = NULL;
		}
	...
	internal_printf("\n=== Summary ===\n");
	internal_printf("Total hosts: %d\n", totalHosts);
	internal_printf("Successful: %d\n", successCount);
	internal_printf("Failed: %d\n", failCount);

go_end:
	if (hostnameList)
	{
		intFree(hostnameList);
	}
